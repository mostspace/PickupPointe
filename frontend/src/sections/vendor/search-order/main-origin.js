...
// @mui
import {FormControl, Button, Typography, TextField, Box,} from '@mui/material';
...
export default function Main() {
  ...
  return (
    <>
      <div className={`w-full h-full flex flex-col gap-[16px] sm:gap-[16px] ${orderList.result.length <= 0 && "justify-between"}`}>
        <div className="flex flex-col items-center gap-[16px] p-[24px] sm:p-[48px] bg-[#f9f9fa] rounded-[24px]">
          <Typography variant="h2" className="text-[26px] sm:text-[32px]">Search your order</Typography>
        ...
        {/* Searched Items */}
        {!isLoading && orderList?.result.length > 0 && orderList.result.map(item => (
                  ...
                  <div className="flex flex-col xs:flex-row justify-between xs:items-center gap-[14px] xs:gap-0">
                    ...
                    <div className="w-fit flex relative cursor-pointer" onClick={handlePickupQRScannedOpen}>
                      <Typography variant="subtitle2" className="underline absolute top-[40%] left-[12%]">Show QR
                        code</Typography>
                      <img src={blurQrImg} className="w-[124px]"/>
                    </div>
      ...
      {/*<React.Fragment>
        ...
        >
          <DialogTitle id="" className='pt-[32px] sm:!pt-[64px] flex flex-col gap-[6px] items-center'>
            <h1
              className='text-[23px] sm:text-[32px] text-heading font-normal sm:leading-[44px] text-center font-gilroy'>Order
              #PP86307332 QR code</h1>
          </DialogTitle>
