...
const feedbackTypes = [
  { label: "Sales Request", value: 1 },
  { label: "Partner Request", value: 2 },
  { label: "Feedback", value: 3 },
  { label: "Support ticket", value: 4 },
];
...
const Main = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/api/v1/vendor/send-feedback`, {
        name: data.name,
        email: data.email,
        role: 'Vendor',
  ...
  return (
    <div className="flex flex-col gap-[32px]">
      ...
      <div className="flex flex-col gap-[32px] border p-7 w-full flex flex-col rounded-xl">
        <Typography variant="h6" className="font-gilroyMedium">Vendor support</Typography>
