...
export const getSearchOrders = createAsyncThunk('vendor/getSearchOrders', async (query) => {
  try {
    if (query === "") {
      return {result: [], totalCount: 0, page: 0}
    }
    const { data } = await axiosInstance.get(`${BASE_URL}/api/v1/vendor/search_order?query=${query}`);
    return data;
  } catch (error) {
